...
**지금 상태**
- 리뉴얼 단계 1·2·3 코드 반영. 커밋은 사용자가 한다 — 새 세션에서 `git status`로 무엇이 안 올라갔는지 사용자에게 먼저 묻는다
- 단계 3 뒤 사용자가 컬럼 드래그가 망가졌다고 했다. 고친 것(커밋 대상):
  - 끌어 넣는 컬럼이 어두운 반투명 판이 되던 것 → 불투명 면 + 두 겹 링 (`.widget-drop-target`, index.css). 원인은 단계 3의 잉크 강조색
  - 드롭 선에 검은 그림자 제거, 꺼낼 자리(`.drop-spot`) 면을 반투명 카드 면으로
  - 카드 아래쪽 절반에 놓아도 위로 들어가던 것 → 가운데 기준 (`dropIndex`, columns.ts). 리뉴얼 전부터 있던 버그
  - 같은 컬럼에서 아래로 끌면 한 줄 아래로 들어가던 것 → 화면 목록 기준으로 칸 계산 (`columnSlotAt`, spaceStore.ts). 리뉴얼 전부터 있던 버그
  - 브라우저·웹앱을 꺼내면 기본 크기(900×620)로 커지던 것 → 넣기 전 제 크기로 (`asPage`, spaceStore.ts). 리뉴얼 전부터 있던 버그

**사용자 결정 대기 — 이것부터 묻는다**
1. **강조색.** 결정 4(잉크)를 사용자가 다시 의심한다("검은색이 맞는지 모르겠다"). 비교 이미지를 보냈다: 잉크 `#141414` / 옛 앰버 `#a9581b` / Tomato `#e8553a` / Cobalt `#3552c8`(밝은 극성 값, 방 화면 선택·체크박스와 컬럼 드롭 화면). 잉크는 본문·테두리와 섞여 선택·드래그 상태가 가장 약하다. 고르면 index.css `:root`(어두움)·`[data-polarity='light']`(밝음)의 `--accent*` 네 값을 바꾸고 결정 4를 고친다. 어두운 극성 값은 새로 정해야 한다
2. **타이머 휴식 모드 숫자 색** (결정 15). 강조색이 정해지면 같이 정한다

**그다음** 리뉴얼 단계 4(색 마크 옅은 채움)부터 순서대로.
...
| 4 | 강조색 | **잉크(검정).** 밝음 `#141414` / 어두움 `#f4f4f5` | 강조색 · B |
...
| 15 | **안 정함** — 타이머 휴식 모드 숫자 색 | 휴식(BREAK)일 때 숫자가 `--accent`인데 이제 본문 잉크와 같아서 집중/휴식이 색으로 안 갈린다. 온보딩 FirstSteps 색종이(강조색·강조 hover·잉크·보조 잉크 순환)도 전부 무채색이 된다(앱에서 안 띄워 봄). 사용자에게 묻는다 | — |
