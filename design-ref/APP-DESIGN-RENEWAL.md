...
**지금 상태**
- 리뉴얼 단계 1·2·3·4 코드 반영, 사용자 확인 끝. 브랜치 `design-review`. 커밋은 사용자가 한다 — 새 세션에서 `git status`로 안 올라간 것이 있는지 먼저 본다
...
**그다음** 리뉴얼 단계 5(사진 위젯)부터 순서대로. 색 마크가 있을 때 6px 여백을 두는 것도 단계 5에 들어 있다(결정 17).

**단계와 따로 남은 제안 (사용자 답 대기)**
- 웹페이지 권한 창(google.com 위치 권한 등)이 `npm run dev`마다 뜬다. 답을 메모리에만 두고 앱을 끄면 잊기 때문이다([electron/main.ts](../electron/main.ts) `permissionAnswers`). 답을 디스크에 저장하자고 제안만 했다
