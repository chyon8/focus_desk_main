...
- 단계 3 뒤 사용자가 컬럼 드래그가 망가졌다고 했다. 고친 것(커밋 대상):
  ...
  - 페이지(webview) 위에서 놓으면 넣기·빼기가 안 되고 드롭 표시만 남던 것 → 끄는 동안 webview 포인터를 끈다 (`.widget-dragging`, Canvas.tsx·index.css). 리뉴얼 전부터 있던 버그

**강조색은 옛 앰버로 되돌렸다 (2026-09-14, 사용자 결정).** 잉크·Tomato·Cobalt와 비교해서 골랐다. index.css `--accent*` 네 값을 리뉴얼 전 값으로 되돌렸다. 결정 4·15 참조
