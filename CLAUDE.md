...
- ❌ `npm run dev`, `npm run dist`, `npx electron ...`, CDP 원격 디버깅, `capturePage` 스크린샷 검증 — **사용자가 명시적으로 "띄워봐"라고 할 때만**
...
- 이 프로젝트에서 자주 어긴 이유: 옛 결정들(CDP로 검증·D-034 capturePage로 배경 확인)이 "실행해서 눈으로 확인"을 표준 검증법으로 적어둔 탓. **그 문서들은 기록이지 지시가 아니다**
