...
| # | 할 일 | 왜 |
|---|---|---|
| 10 | **로봇 인증 화면이 계속 뜬다 — 근본 해결 ← 다음** | 로그인·웹앱이 이 앱의 핵심인데 인증에서 막힌다 |
...
- 사이트(사용자, 2026-09-13): 구글, 프로덕트 헌트, 그 밖에도 많다
- 먼저 할 것: 각 사이트에서 어떤 인증(reCAPTCHA / Cloudflare Turnstile / 구글 `/sorry`)이 뜨는지 확인한다. 같은 사이트를 실제 Chrome에서 열어 비교한다
