...
- 2026-08-17: **Phase D — 창 크기 기억**(D-044). 종료·창 닫기로 데스크가 사라질 때 렌더러 cleanup이 안 돌아 앱 창이 위젯 크기로 영구히 남던 것을 `before-quit`/`close` 복원으로 막음

## 다음 할 일
1. **사용성 QA** ← 여기부터. UX가 최선인지, 실사용 오류 없는지 훑는 라운드
2. **앱 서피스 나머지**(전부 [APP-SURFACE.md](APP-SURFACE.md) 리스크 표에 상세) — 창 여러 개 바인딩(제목 기억, 필요성 재확인 후) / 공간 진입 시 자동 실행 / 썸네일 1fps→SCStream / 배포 시 헬퍼 서명·권한 재승인. FL Studio류 리사이즈 거부는 해결 불가로 확정
3. **배포 마무리**(D-027) — Developer ID 인증서 → 공증 + 자동 업데이트 (아이콘 완료)
