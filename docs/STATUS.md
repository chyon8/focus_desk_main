...
| # | 할 일 | 크기 | 끝난 기준 |
|---|---|---|---|
| 17 | **로그인 정리 — 17-4만 남음** (로그인 분리를 위젯 단위로) | 큼 | 세부 계획 컨펌 → 구현. 아래 17번 |
...
## 17. 로그인 정리 (2026-09-17 사용자 확정)

**17-1~17-3은 끝났다 (2026-09-17, 확인용 앱에서 봄).** 새 공간은 Cloud 단색·밝은 UI로 열린다(`newSpace`). 로그인 목록 이름은 "Sites with saved data"다. 구글 차단 화면(`/sorry/`)을 받으면 1초 뒤 원래 주소를 한 번만 다시 연다(`BrowserWidget`의 `blockRetried`) — 확인 때 이 IP가 실제로 막혀 있어서, 다시 열린 뒤 또 막혔고 거기서 멈췄다. **남은 것은 17-4다. 세부 계획을 컨펌받은 뒤 코딩한다.**
