...
| 15 | **공간 메뉴 여는 방법 바꾸기** — 조사 끝, 방식 결정 대기 | 중간 | 사용자가 고른 방식으로 이름 바꾸기·삭제·Sign-ins를 연다. 아래 15번 |
...
> 15는 2026-09-17에 추가했다. 13 다음 자리는 임시다 — 13의 B1보다 먼저 할지 사용자가 정한다.
...
**D(측정), A1(같은 프로필 두 번 실행 막기), A2(검사 페이지 주소 저장 안 하기), A3(로그인 공유)은 끝났다 (2026-09-17, 다음은 B1).** A2는 주소 규칙(`addressToSave`)에 레딧 검사 파라미터·`/cdn-cgi/`를 더하고, 검사 제목(`isCheckTitle`)이 뜨면 리다이렉트 전 주소를 저장한다. A3는 새 공간이 `persist:shared`를 쓰고, 기존 공간은 `separate`로 옮겼다 — Sign-ins 패널의 "Keep this space's sign-ins separate"로 바꾼다. **사용자 쪽 할 일: 지금 쓰는 공간들은 아직 따로다. 공유로 쓰려면 공간마다 이 버튼을 끈다** 고친 뒤에 같은 방법으로 다시 재서 아래 "전"과 비교한다.
...
## 15. 공간 메뉴 여는 방법 바꾸기 — 조사 (2026-09-17, 결정 대기)

**문제 (사용자):** 이름 바꾸기·삭제·Sign-ins가 "지금 공간 썸네일을 한 번 더 누르기"로만 열린다([Rail.tsx](../src/app/Rail.tsx) `SpaceTile`의 onClick). 보이는 표시가 없고, 다른 공간은 그 공간으로 옮겨가야만 메뉴를 열 수 있다. 우클릭·더블클릭은 없다. 사용자가 "예전부터 이상했다"고 했다.

**다른 앱이 하는 방식**
- Arc: 공간 이름에 마우스를 올리면 Edit가 보이고, 거기서 이름·테마·**프로필(로그인)**·삭제를 한 메뉴에서 바꾼다. 공간 아이콘 우클릭 → Delete Space. 제목을 누르면 바로 이름 편집
- Notion·Slack·Figma·VS Code: 항목 우클릭 = 호버 때 보이는 `…` 버튼과 같은 메뉴. 이름은 더블클릭(또는 F2)으로 그 자리에서 편집
- NN/g: 우클릭 메뉴는 모르는 사람이 많으니 `…` 같은 보이는 표시나 다른 경로를 같이 둔다. 자주 쓰는 동작을 괜히 숨기지 않는다

**안**
| 안 | 내용 | 비고 |
|---|---|---|
| A | 모든 공간 타일에 우클릭 메뉴만 추가 | 가장 작다. 여전히 안 보인다 |
| B (추천) | 우클릭 메뉴(모든 타일, 전환 없이) + 호버 때 타일 모서리 `…` 버튼(같은 메뉴) + 타일 아래 이름 더블클릭으로 바로 편집. 메뉴 안 Sign-ins는 Shared/Separate를 바로 바꾸는 항목 + "Manage sign-ins…" | Arc·Notion과 같은 조합. 레일 폭 60px이라 `…`는 작게(16px) |
| C | 공간 설정 패널 하나(이름·배경·Sign-ins·삭제)를 레일 도구 줄에 둔다 | Arc의 Edit와 비슷. 패널이 하나 늘어난다 |

**착수 전에 사용자에게 받을 것:** 안 선택 / 지금의 "한 번 더 누르기"를 없앨지 남길지(없애는 건 기존 기능 제거라 먼저 묻는다) / 순서(13의 B1보다 먼저 할지)
