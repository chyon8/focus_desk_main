...
- 2026-08-17: **창 배정 UI**(D-048) / **최대화 위젯 줌 역보정**(D-049) / **시간 저장본 2배 증식 버그**(D-050, StrictMode 이중 load — 오염 값은 로드 시 폐기)
...
## 다음 할 일
1. **D-044~050 실기 검증** ← 여기부터. 앱 위젯 얼굴(아이콘)·창 배정 UI·공간 진입 자동 실행·종료 시 창 크기 복원·최대화 중 줌·시간 값 정상화. 사용자가 로컬 빌드로 며칠 써보기로 함
2. **사용성 QA** — UX가 최선인지, 실사용 오류 없는지 훑는 라운드 (앱 서피스 검증 후)
3. **배포 마무리**(D-027) — Developer ID 인증서 → 공증 + 자동 업데이트. 헬퍼 바이너리도 서명 대상
4. 미검증: ⌥Space 왕복(위젯 클릭 왕복은 확인됨), 60fps 실측, 앰비언스 소리 품질 / 정리: `legacy/` 삭제
