...
| # | 할 일 | 왜 |
|---|---|---|
| 10 | **로봇 인증 — 사용자 확인 대기 ← 다음** | 구글 차단 페이지 원인은 고쳤다(2026-09-13). 실제 앱에서 music·test2 공간을 열어 확인만 남았다 |
| 12 | 즐겨찾기·온보딩 앱 목록 수정 — 런칭 전 | 사용자가 34개 목록([presets.ts](../src/webapps/presets.ts))에 이상한 게 있다고 했다(2026-09-13). 무엇을 고칠지는 사용자에게 받는다 |
| 8 | 온보딩 디자인 검토 | 첫 화면을 A(전체 화면 + 썸네일 줄)로 바꿨다(2026-09-13). 10·12 뒤에 한 번 |
...
### 10-10. QA (2026-09-13 사용자가 짚은 것 — 하나도 빼지 말 것)

전부 코드만 읽고 추정한 것이다. **고치기 전에 테스트 창(test 프로필 + CDP)으로 재현부터 한다** — 방법은 메모리 `electron-cdp-verify`.

**10-10. 로봇 인증 (2026-09-13 고침, 사용자 확인 대기)**
- 원인: 브라우저 위젯이 구글 차단 페이지(`/sorry/index?continue=…`) 주소를 그대로 저장했다. 그 주소에는 발급 당시 IP·시각에 묶인 토큰이 있어서, 공간을 열 때마다 다시 로드되면 항상 차단 페이지가 뜬다. 사용자 프로필의 music·test2 공간이 그 상태였다
- 고친 것: [browserAddress.ts](../src/widgets/browserAddress.ts) `addressToSave` — 차단 페이지는 `continue` 주소로, 구글 검색은 방문마다 바뀌는 파라미터를 빼고, Cloudflare `__cf_chl_*`는 빼고 저장한다. 이미 저장된 나쁜 주소는 위젯이 열릴 때 고친다
- 테스트 창에서 본 것: 사용자의 저장된 차단 주소로 위젯을 만들면 검색 결과가 뜨고 저장 주소가 깨끗해진다. sannysoft 전부 통과, reCAPTCHA v3 0.9, Cloudflare 테스트 통과
- 남은 후보(확정 아님): Electron 39라 크롬 142로 보인다(지금 크롬 152). Electron 44.3.0이 크롬 152. 인증이 다른 사이트에서 또 뜨면 이것부터 본다
