...
| # | 할 일 | 크기 | 끝난 기준 |
|---|---|---|---|
| 12 | **정렬 다시 만들기 — 구현 대기** | 중간 | Compact·Rows 두 모드가 위젯 크기를 안 바꾸고, 정렬 뒤 자동 맞춤이 100%를 안 넘는다. 구현은 착수 요청 뒤에 시작한다 |
...
| 19 | 작은 브라우저 화면 | 중간 | 손으로 좁힌 브라우저에서 시작 페이지와 도구줄이 쓸 만하다. 12번 다음 |
| 11 | 서명·공증 | — | **사용자만 가능.** 하드 블로커 |

> 순서 13 → 14는 2026-09-16 사용자가 정했다. 12(정렬)는 2026-09-19에 기획을 다시 써서 올렸다. 문서에는 기획만 있고, 구현은 별도 착수 요청 뒤에 진행한다.
...
**정렬은 자리만 바꾼다. 위젯 크기는 건드리지 않는다.** 정렬 전 브라우저가 900×620이면 정렬 후에도 900×620이다. 좁은 칸에 맞춰 위젯을 줄이면 헤더·아이콘은 커 보여도 본문이 쓸 공간이 줄어든다. 작은 위젯을 큰 칸 폭으로 늘리는 것도 하지 않는다.

**G를 누르면 크기와 순서를 유지한 채 정리하고, 최대 100% 안에서 대상 전체를 보여준다.**

이 장의 결정은 2026-09-19 대화에서 정했다. 구현은 별도 착수 요청 뒤에 시작한다.

### 12-1. 모드 둘

| 항목 | Compact (촘촘히) | Rows (줄 맞춤) |
|---|---|---|
| 집는 순서 | 현재 위치 `y → x → id` | 동일 |
| 놓는 자리 | 순서대로 집어서, 놓을 수 있는 가장 위·왼쪽 빈자리 | 한 줄씩 왼쪽부터, 윗변 맞춤 |
| 크기 | 현재 폭·높이 그대로 | 동일 |
| 남는 자리 | 구멍을 남기지 않는다 | 짧은 위젯 아래는 비워 둔다 |
| 열 수 | 자동 | Auto·1~5 설정 유지 |

- **Compact는 구멍 없이 채우는 쪽을 택한다. 늦게 집힌 위젯이 먼저 집힌 것보다 위에 놓여도 된다** (2026-09-19 결정). 순서를 지키는 쪽은 Rows다 — Compact까지 순서를 지키면 두 모드가 같은 그림이 된다.
- 기본 간격은 캔버스 좌표 **16px**, 화면 바깥 여백은 최소 20px. 이 16은 `tidy.ts`의 자체 상수다. `ARRANGE_GAP`(32)은 새 위젯 놓기·복제·컬럼에서 계속 쓰이므로 그대로 둔다.
- 공통 열 폭에 맞춰 위젯을 늘리지 않는다. 작은 위젯을 큰 칸 가운데 놓지 않는다 — 왼쪽·윗변에 붙인다.
- 위젯이 적거나 크기가 비슷해 두 모드 결과가 같아도 그대로 둔다. 차이를 내려고 크기나 간격을 바꾸지 않는다.

### 12-2. 계산 — `src/canvas/tidy.ts`

- 새 파일로 만든다. `layout.ts`의 `arrange`·`ARRANGE_GAP`은 온보딩·크롬 가져오기가 함께 쓰므로 건드리지 않는다.
- 입력은 위젯의 현재 위치·크기, 가용 화면 크기, 모드, 열 수. **출력은 위젯별 `{x, y}`만.** 폭·높이를 돌려주지 않는다 — 반환 타입으로 막는다.
- 클릭 순서와 현재 배율은 계산에 영향을 주지 않는다.
- Rows Auto: 한 줄에 1개부터 N개까지 놓은 후보를 만들어 비교한다.
- Compact: 목표 가로폭 후보는 ① Rows 후보들의 전체 가로폭 ② 가용 화면 폭. 가장 넓은 위젯보다 좁은 후보는 버린다. 이미 놓인 위젯들의 오른쪽·아래 경계에 간격을 더한 지점을 검사해 첫 빈자리에 놓는다.
- 후보 선택: 전체를 가장 크게 보여주는 것 → 동률이면 경계 면적이 작은 것 → 그다음 가로폭이 작은 것.

### 12-3. 줌·선택·Undo

- 정렬 뒤 전체 경계와 가용 화면으로 배율을 구하고 **100%를 상한**으로 자른다. 자른 배율 기준으로 중심도 다시 잡는다. 레일·상단 영역은 빼고, 선택 도구막대가 떠 있으면 그 영역도 피한다.
- 1~2개가 다 들어오면 100%로 둔다. 화면을 채우려고 더 확대하지 않는다. 최소 배율 10%는 그대로 두고, 그보다 작아야 전부 들어오는 배치는 10%에서 캔버스 이동으로 본다.
- **100% 상한은 G 직후 자동 맞춤에만 적용한다.** 수동 줌과 페이지 줌은 제한하지 않는다. 캔버스 줌은 브라우저의 실제 폭·높이와 페이지 줌을 바꾸지 않는다.
- 선택이 있으면 선택한 것만, 기존 선택 영역의 왼쪽 위를 기준으로 정리한다. 나머지는 움직이지 않는다.
- 컬럼은 자식을 포함한 하나의 대상이다(`inPlay`가 이미 그렇게 한다). 컬럼 크기 규칙은 그대로 둔다.
- 위치와 카메라는 한 번에 반영한다. 반복 G에서 결과가 같으면 저장도 Undo 기록도 새로 만들지 않는다. Undo는 이전 위치·배율·모드를 되돌린다.
- 위젯이 최대화된 상태에서의 G는 지금 동작을 유지한다.

### 12-4. 메뉴와 저장

- `ArrangeMode`를 `'compact' | 'rows'`로 바꾼다. 기본은 Compact, G는 그 공간에서 마지막에 쓴 모드를 반복한다. 페이지 안에서는 ⇧G(이미 반영됨).
- `SCHEMA_VERSION`을 17로 올리고 [migrate.ts](../src/spaces/migrate.ts)에 변환을 더한다: `grid`·`stack` → `rows`, `masonry` → `compact`. 열 수는 Rows에서만 유지하고, 설정이 없으면 Compact.
- 변환은 정렬 설정만 바꾼다. 위젯의 위치·크기는 G를 누르기 전까지 그대로 둔다.
- [Dock.tsx](../src/app/Dock.tsx) `MODES`를 두 개로 줄이고, 열 수 버튼은 Rows를 고른 동안만 보이게 한다.
- 온보딩·크롬 가져오기의 초기 배치는 기존 경로를 그대로 둔다.

### 12-5. 검증

자동 테스트로 판정하는 합격선 셋:

1. 정렬 전후 모든 위젯의 폭·높이가 같다
2. 이웃한 위젯 사이 간격이 전부 16이다
3. 같은 모드를 두 번 눌러 좌표와 카메라가 같다

눈으로 보는 것 (1280×800 · 1440×900 · 1920×1080):

- 위젯 1개·2개, 서로 다른 6개·12개, 브라우저 2개+타이머+빈 컬럼
- 손으로 크기를 바꾼 위젯, 높이가 긴 컬럼, 일부만 선택해서 정렬
- 50%·100%·200%에서 시작해 G, 반복 G, 모드 전환, Undo, 앱 재시작
- **위젯 12개일 때 정렬 후 배율을 적어 두고 받아들일 만한지 판단한다.** 크기를 유지하면 12개는 100%에 안 들어간다 — 한 줄로 펼치면 가로 2700 근처가 된다
- 900×620 브라우저가 정렬 후에도 900×620이고, 정렬 때문에 주소줄 표시나 시작 페이지 열 수가 바뀌지 않는다
- 빈자리 추가·페이지 줌·로그인 선택·컬럼·기존 초기 배치가 그대로다

### 범위 밖

- **기존 정렬로 작아진 위젯의 크기 복구는 하지 않는다** (2026-09-19 사용자 — 가져갈 데이터가 없다). 새로 만들면 된다.
- 작은 브라우저 화면은 19번으로 뺐다. 정렬이 브라우저를 좁히지 않게 되면 급하지 않다.
