...
- **정렬(G)은 자리만 바꾼다**([tidy.ts](../src/canvas/tidy.ts)): Compact·Rows 두 모드, 간격 16, 출력은 `{x, y}`뿐이라 위젯 크기를 못 바꾼다. 직후 카메라는 100%가 상한. 칸 크기에 맞춰 다시 그리는 `layout.ts`의 `arrange`는 온보딩·크롬 가져오기 전용이다
