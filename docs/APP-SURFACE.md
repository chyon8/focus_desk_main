...
| `WAITING` | 원래 크기로 되돌아가 있음 | "위젯을 캔버스 안으로" 안내 |
...
- **딱 하나의 규칙: 위젯이 캔버스 안에 온전히 들어와 있을 때만 창이 붙는다.** 실제 창은 우리가 그리는 게 아니라 잘라낼 수가 없다 — 가장자리에 걸치거나 사이드바 밑으로 들어가면 창을 원래 자리로 돌려보내고(`WAITING`), 다시 들어오면 다시 붙인다. 경계 여유는 부착 2px·유지 8px, 그리고 **벗어난 상태가 400ms 이어져야** 내보낸다(사이드바 애니메이션이 창을 뺏지 않도록)
- **팬은 `move`(위치만), 줌은 `place`(크기까지, 카메라가 멈춘 뒤 1회).** `AXSize` 쓰기는 앱이 인터페이스를 통째로 다시 잡게 만들어 비싸고, `AXPosition`은 싸다
- **라이브 썸네일은 폐기**(D-047) — 화면기록 권한도 같이 사라짐
- **숨기지 않고 뒤로 보낸다**: 창을 숨기는 게 아니라 데스크를 앞으로 올리는 것(D-040)
- **전체화면(⌃⌘F) 중에도, 창 모드에서도 데스크 창 레벨을 일반으로 내린다**(D-051·067). 안 내리면 앱이 그 위로 못 올라와 왕복이 죽는다 — 창 모드에서 이게 실제로 일어났다(`setVisibleOnAllWorkspaces`가 레벨을 올린다)
- **창을 직접 옮기거나 키우면 위젯이 따라간다**(헬퍼가 0.3초마다 프레임 확인). 캔버스를 벗어나면 `AWAY` — 창은 그대로 두고 슬롯만 놓아준다 (D-067)
- 이탈: 위젯의 "Send back" / 공간 전환 / 위젯 삭제 / 종료
- **앱이 하나라도 열려 있는 동안 데스크는 창 레벨 −1(바닥)이다**(D-067). 그래서 데스크를 클릭해도 앱 창이 뒤로 가지 않는다 — 앱은 위젯 위에 계속 떠 있고 데스크는 바닥이다. `⌥Space`는 두 겹 토글(데스크를 통째로 앞으로 ↔ 다시 바닥으로), 위젯 표면 클릭은 그 앱만 앞으로 (D-042)
...
src/apps/useAppSurface.ts                     위젯이 캔버스에 온전히 보이는 동안 실제 창 배치 —
                                               팬=move 60ms, 줌=place 140ms 정지 후 (D-067)
