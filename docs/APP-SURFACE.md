...
- **딱 하나의 규칙: 위젯이 캔버스 안에 온전히 들어와 있을 때만 창이 붙는다.** 실제 창은 우리가 그리는 게 아니라 잘라낼 수가 없다 — 가장자리에 걸치거나 사이드바 밑으로 들어가면 창을 원래 자리로 돌려보내고(`WAITING`), 다시 들어오면 다시 붙인다. 경계에서 떨리지 않게 6px 여유
