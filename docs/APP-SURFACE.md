...
| **C** | `place`·`restore`·`raise`. 최대화 시 실제 창이 위젯 자리로, 왕복 가능 | 접근성 | ✅ 구현, ⚠️ **⌥Space/재클릭 왕복은 미검증**(D-042, 다음 세션 1번) |
...
| **LIVE 중 Focus Desk 클릭 → 앱이 뒤로 감** | z-order를 공유하지 않는 두 창의 근본 한계, 막을 수 없음. `⌥Space` 전역 단축키 + 위젯 재클릭으로 왕복(D-042) — **다음 세션에서 실기 검증 필요** |
