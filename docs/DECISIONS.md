...
> 형식: 날짜 / 결정 / 이유 / 버린 대안. 최신순. 번호는 재사용하지 않는다(다른 문서·코드 주석이 참조). 뒤집을 때는 새 항목을 추가하되, **완전히 죽은 결정**(코드에 흔적이 없고 뒤집은 항목이 이유까지 흡수한 것)은 정리한다 — 2026-08-17에 8개 삭제(D-004·010·015·017·019·024·025·026).
...
- `npx electron dist-electron/main.js --remote-debugging-port=9222` → `http://localhost:9222/json`의 **file:// 타겟**(브라우저 위젯 페이지가 섞이므로 필터링)에 WebSocket으로 `Runtime.evaluate`. 실제 DOM을 클릭/입력해 스토어·영속화·IPC를 한 번에 검증. React 제어 input은 `HTMLInputElement.prototype.value` 네이티브 setter + `input` 이벤트 dispatch
- ⚠️ **AI는 이 방법을 쓰지 않는다**(CLAUDE.md §0-1). 앱 실행은 사용자 몫이고, 이 항목은 사용자가 쓸 기록이다
