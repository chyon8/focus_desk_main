...
## D-046 (2026-08-17) 공간 진입 = 그 공간의 앱을 배경으로 실행 (Phase D 나머지)
공간은 프로젝트이므로, 공간을 여는 것은 그 프로젝트의 앱을 여는 것이다. 앱이 안 떠 있는 위젯은 클릭하기 전까지 죽은 카드다

- `useSpaceApps`(이미 공간의 앱 집합을 main에 보내는 훅)에서 각 앱에 `launch(activate: false)`를 보낸다. 시간 판정용 집합 전송과 같은 트리거라 새로 관리할 상태가 없다
- **`activates = false`가 핵심.** 공간으로 전환한 이유는 그 공간에 있으려는 것이지 마지막으로 뜬 앱 안에 있으려는 게 아니다. 활성화까지 하면 전환할 때마다 포커스를 빼앗긴다
- 이미 실행 중이면 `NSWorkspace.openApplication`이 사실상 아무 일도 하지 않는다 — "실행 중인지 먼저 확인"이 따로 필요 없다
