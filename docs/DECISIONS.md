...
> 형식: 날짜 / 결정 / 이유 / 버린 대안. 최신순. 번호는 재사용하지 않는다(다른 문서·코드 주석이 참조).
> 뒤집을 때는 새 항목을 추가하되, **완전히 죽은 결정**(코드에 흔적이 없고 뒤집은 항목이 이유까지 흡수한 것)은 정리한다 — 2026-08-17에 8개 삭제(D-004·010·015·017·019·024·025·026).
...
- 주의: 브라우저 위젯(`<webview>`)이 떠 있으면 타겟 목록에 그 페이지도 섞이므로 file:// 로 필터링할 것
